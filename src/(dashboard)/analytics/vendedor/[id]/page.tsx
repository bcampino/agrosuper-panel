...
export default async function VendedorDetailPage({ params, searchParams }: PageProps) {
  ...
  // Fetch locations assigned to this vendedor
  const { data: locs } = await supabase
    .from('locations')
    .select('id, name, code, region, sa_status')
    .eq('staff_vendedor_id', id)
    .eq('is_active', true)
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Vendedor · {monthLabel}</p>
          </div>
          <Suspense>
