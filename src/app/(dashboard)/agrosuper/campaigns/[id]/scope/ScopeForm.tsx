...
import { Upload, FileSpreadsheet, CheckCircle, XCircle, ArrowLeft, Loader2, Info } from 'lucide-react'
...
export default function ScopeForm({ campaignId, campaignName, baseNumber }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    body.append('base_number', String(baseNumber))

    try {
      const res = await fetch('/api/agrosuper/upload-scope', { method: 'POST', body })
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Subir Base de Locales</h1>
      ...
      <div className="rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: '#f0f8ff', borderLeft: '4px solid #007BFF' }}>
        ...
        <p className="text-gray-500 text-xs mt-0.5">Base {baseNumber} — Scope (locales a visitar)</p>
      </div>

      <form onSubmit={handleSubmit}>
        <Card className="p-6 space-y-5">
          {/* Info box */}
          <div className="bg-blue-50 border rounded-lg px-3 py-3 flex gap-3 text-sm" style={{ borderColor: '#007BFF' }}>
            <Info className="h-5 w-5 shrink-0 mt-0.5" style={{ color: '#007BFF' }} />
            <div style={{ color: '#0c5aa8' }}>
              <p className="font-semibold mb-1">Formato esperado:</p>
              <p className="text-xs">Excel con columnas: ID, Nombre, Dirección (opcional), Región (opcional)</p>
            </div>
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Excel</label>
            <label
              className={`flex flex-col items-center justify-center w-full h-36 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
                file ? 'border-[#FF740C] bg-orange-50' : 'border-gray-300 bg-gray-50 hover:border-[#007BFF] hover:bg-blue-50'
          ...
          >
            {loading ? (
              ...
              <>
                ...
                Importar Base {baseNumber}
              </>
            )}
          </button>

          <p className="text-xs text-gray-400 text-center">Proceso de ~2 a 10 segundos según el tamaño del archivo.</p>
        </Card>
      </form>
      ...
      {result && (
        <Card className={`p-4 flex items-start gap-3 border ${result.success ? 'bg-orange-50 border-[#FF740C]' : 'bg-red-50 border-red-200'}`}>
          ...
          <div>
            {result.success ? (
              <>
                <p className="text-sm font-semibold" style={{ color: '#1E2C3E' }}>
                  Base {baseNumber} importada para {result.campaignName}
                </p>
                <p className="text-xs mt-1" style={{ color: '#666' }}>
                  {result.imported} locales importados de {result.total} filas.
                </p>
